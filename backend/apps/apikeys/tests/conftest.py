...
@pytest.fixture
def api_key_client(db):
    """An APIClient authenticated with an API key (tenant resolved from the key)."""
    def _make(tenant, scopes=(Scope.READ,)):
        key, raw = ApiKey.generate(tenant=tenant, name="test", scopes=list(scopes))
        api = APIClient()
